...
BEGIN
	...
	-- ----- STOP: Transaction Begin -----
		...
		IF (@parmKommentar IS NULL AND ((SELECT [Environment] FROM [utl].[Environment]) = 'UDV')) 
		BEGIN
			SET @Return		=	0
			raiserror('Fejl: Der er ikke angivet kommentar for aktivering, dette skal gøres i Udvikling', 18, 1)	
		END
      ...
      SET @parmKommentar = ISNULL(@parmKommentar,'')
		...
		EXEC [flw].[SkrivJobListeKommentar] @parmJobID = @parmJobID, @parmKommentar = @parmKommentar
