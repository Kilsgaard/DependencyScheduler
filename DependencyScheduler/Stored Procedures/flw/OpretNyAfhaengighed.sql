...
-- Description:	Procedure til oprettelse af ny afhængighed
-- =============================================
CREATE PROCEDURE [flw].[OpretNyAfhaengighed]
...
BEGIN
	...
	-- ----- STOP: Transaction Begin -----
		...
		BEGIN		
			SET @Return		=	0
			raiserror('Fejl: Der mangler Child JobID eller JobNavn', 18, 1)	
		END 

		IF (NOT @parmChildJobID IS NULL) AND (NOT @parmChildJobName IS NULL)
		BEGIN
			SET @Return		=	0
			raiserror('Fejl: Både Child JobID og JobNavn er angivet', 18, 1)	
		END
		...
		BEGIN
			SET @Return		=	0
			raiserror('Fejl: Problemer med Child JobID eller JobNavn', 18, 1)		
		END
		...
		BEGIN
			SET @Return		=	0
			raiserror('Fejl: Child Jobbet eksistere ikke i JobListen', 18, 1)		
		END
		-- ----- STOP: Validering Job Exists -----

		-- ----- Start: Code -----
		INSERT INTO [flw].[JobAfhaengighed]
						(
						  [ParentJobID]
						 ,[ChildJobID]
						)
